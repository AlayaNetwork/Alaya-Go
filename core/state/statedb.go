...
func (s *StateDB) SetState(address common.Address, key, value []byte) {
	...
	if stateObject != nil {
		if s.Gte0140VersionState() {
			stateObject.SetState(s.db, key, stateObject.getPrefixValue(s.originRoot.Bytes(), key, value))
		} else {
			stateObject.SetState(s.db, key, stateObject.getPrefixValue(nil, key, value))
		}
	}
	s.lock.Unlock()
}
...
func (s *StateDB) Gte0140VersionState() bool {
	if !GovGte0140Version {
		GovGte0140Version = Gte0140Version(s.GetCurrentActiveVersion())
	}
...
func (s *StateDB) ListActiveVersion() ([]gov.ActiveVersionValue, error) {
	//avListBytes := s.GetState(vm.GovContractAddr, gov.KeyActiveVersions())
...
func (s *StateDB) getStateObjectCache(addr common.Address) (stateObject *stateObject) {
	...
	for parentDB != nil {
		...
		if obj != nil {
			...
			//	panic(fmt.Sprintf("executing statedb clearref is not empty:%d, root:%s", len(s.clearReferenceFunc), parentDB.Root().String()))
			//}
...
func (s *StateDB) ClearIndexReference(index int) {
	...
	if len(s.clearReferenceFunc) > index && s.clearReferenceFunc[index] != nil {
		//fn := s.clearReferenceFunc[index]
		//fn()
		log.Trace("Before clear index ref", "reflen", len(s.clearReferenceFunc), "index", index)
		//s.clearReferenceFunc = append(s.clearReferenceFunc[:index], s.clearReferenceFunc[index+1:]...)
