...
func NewBackoffConnector(h Host, cacheSize int, connectionTryDuration time.Duration, backoff BackoffFactory) (*BackoffConnector, error) {
...
func (c *BackoffConnector) Connect(ctx context.Context, peerCh <-chan *enode.Node) {
	for {
		select {
		case pi, ok := <-peerCh:
			if !ok {
			...
			if pi.ID() == c.host.ID().ID() /*|| pi.ID == ""*/ {
				continue
			}

			c.mux.Lock()
			val, ok := c.cache.Get(pi)
			...
			go func(pi enode.ID) {
				...
				if err != nil {
					log.Debugf("Error connecting to pubsub peer %s: %s", pi, err.Error())
					return
				}
			}(pi.ID())
