...
func (srv *Server) Start() (err error) {
	...
	dialer := newDialState(srv.localnode.ID(), srv.StaticNodes, srv.BootstrapNodes, srv.ntab, dynPeers, srv.NetRestrict, srv.MaxConsensusPeers)
	srv.loopWG.Add(1)
	go srv.run(dialer)
	return nil
}
...
func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *enode.Node) error {
	...
	if phs.Version < baseProtocolVersion {
		clog.Error("Low version of p2p protocol version", "err", phs.ID, "version", phs.Version)
