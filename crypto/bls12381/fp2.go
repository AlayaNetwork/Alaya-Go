...
func (e *fp2) fromPointBytes(x, y []byte) (*fe2, error) {
	if len(x) != 48 || len(y) != 48 {
		return nil, errors.New("length of input string should be 48 bytes")
	}
	c1, err := fromBytes(x)
	if err != nil {
		return nil, err
...
func (e *fp2) toPointBytes(a *fe2) ([]byte, []byte) {
	return toBytes(&a[1]),toBytes(&a[0])
}
