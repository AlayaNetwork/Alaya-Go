...
// FromPointBytes constructs a new point given uncompressed byte input.
// FromPointBytes does not take zcash flags into account.

func (g *G2) FromPointBytes(x1,x2,y1,y2 []byte) (*PointG2, error) {
	if len(x1) != 48 || len(x2) != 48 || len(y1) != 48 || len(y2) != 48 {
		return nil, errors.New("input string should be equal 48")
	}
	p0, err := g.f.fromPointBytes(x1, x2)
	if err != nil {
		return nil, err
	}
	p1, err := g.f.fromPointBytes(y1, y2)
	if err != nil {
		return nil, err
	}
	// check if given input points to infinity
...
// ToPointBytes serializes a point into bytes in uncompressed form,
// does not take zcash flags into account,
// returns (0, 0) if point is infinity.
func (g *G2) ToPointBytes(p *PointG2) ([]byte,[]byte,[]byte, []byte) {
	x1,x2,y1,y2 := [48]byte{},[48]byte{},[48]byte{},[48]byte{}
	if g.IsZero(p) {
		return x1[:], x2[:], y1[:], y2[:]
	}
	g.Affine(p)
