...
func (g *G1) FromPointBytes(x,y []byte) (*PointG1, error) {
...
// ToPointBytes does not take zcash flags into account.
// ToPointBytes returns (0, 0) if point is infinity.
func (g *G1) ToPointBytes(p *PointG1) ([]byte,[]byte) {
	p0 := [48]byte{}
	p1 := [48]byte{}
	if g.IsZero(p) {
		return p0[:], p1[:]
	...
	return toBytes(&p[0]), toBytes(&p[1])
}
